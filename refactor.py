...
with open('frontend/src/app/(dashboard)/investments/bonds/page.tsx', 'r', encoding='utf-8') as f:
    code = f.read()

# 1. State changes
state_replace = '''  // For editing and bulk signal grouping
...
code = re.sub(
    r'  // For editing existing signals.*?(?=  const \[settingsOpen, setSettingsOpen\] = useState)',
    state_replace + '\n',
    code,
    flags=re.DOTALL
)
...
save_group_logic = '''  const saveGroupSignals = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      if (editingGroup) {
         // This is an edit
         // Old signals we had in this group:
         const oldSignalIds = editingGroup.signals.map((s: any) => s.id);
         // If condition_type changed, we just delete all old ones, because the condition changed drastically
         if (editingGroup.condition_type !== bulkSignalForm.condition_type) {
            ...
            // They kept condition_type, so we only delete signals for bonds they UNCHECKED
      ...
      // Create/Update the selected bonds (POST /bulk handles upserts if condition_type matches)
...
code = re.sub(
    r'  const updateSignal = async \(e: React.FormEvent\) => \{.*?(?=  const removeSignal = async)',
    save_group_logic + '\n',
    code,
    flags=re.DOTALL
)
...
signals_tab_replacement = '''      {/* SIGNALS TAB */}
      {mainTab === "signals" && (
...
code = re.sub(
    r'      \{/\* SIGNALS TAB \*/\}.*?(?=      \{/\* EDIT SIGNAL MODAL \*/\})',
...
code = re.sub(
    r'      \{/\* EDIT SIGNAL MODAL \*/\}.*?(?=      \{/\* STICKY BULK ACTION BAR \*/\})',
    '\n',
    code,
    flags=re.DOTALL
)

# 5. Modify STICKY BULK ACTION BAR so the Delete All button is hidden in "signals" tab (we deleted the main checkbox anyway but just to be safe)
code = re.sub(
    r'          \{mainTab === "signals" && \(.*?\n          \)\}',
    '',
    code,
...
modal_replace = '''      {/* UNIFIED SIGNAL MODAL */}
      {bulkModalOpen && (
            ...
            <form onSubmit={saveGroupSignals} className="p-5 overflow-y-auto custom-scrollbar flex-1 flex flex-col gap-4">
              ...
              {bulkSignalForm.condition_type !== "news_mention" && (
                <div className="animate-fade-in">
                  ...
                  />
                  <p className="text-[10px] text-[var(--muted)] mt-1.5">При установке абсолютного значения сразу для нескольких бумаг, убедитесь что указанный уровень актуален для каждой из них.</p>
                </div>
              )}
                  ...
                  {portfolio.length === 0 ? (
                    ...
                    <label key={p.bond.id} className={`flex items-center gap-3 cursor-pointer px-3 py-2 rounded-lg transition-colors border ${selectedBonds.has(p.bond.id) ? 'bg-[var(--accent)]/5 border-[var(--accent)]/30' : 'bg-[var(--card)] border-[var(--border)] hover:border-[var(--muted)]'}`}>
                      ...
                      <div className="flex flex-col">
                        <span className="text-[13px] font-semibold text-[var(--foreground)] leading-tight">{p.bond.shortname}</span>
...
code = re.sub(
    r'      \{/\* BULK SIGNAL MODAL \*/\}.*?(?=    </div>\n  \);\n\})',
    modal_replace + '\n',
    code,
    flags=re.DOTALL
)

with open('frontend/src/app/(dashboard)/investments/bonds/page.tsx', 'w', encoding='utf-8') as f:
    f.write(code)

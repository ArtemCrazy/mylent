...
export const api = {
  ...
  investments: {
    ...
    addSignalBulk: (body: { name?: string, bond_ids: number[], condition_type: string, target_value: number | null, news_category?: string, cron_minutes?: number, notify_telegram?: boolean }) => request<{status: string, created: number}>("/investments/signals/bulk", { method: "POST", body: JSON.stringify(body) }),
    updateSignal: (id: number, body: { name?: string, condition_type?: string, target_value?: number | null, news_category?: string, cron_minutes?: number, notify_telegram?: boolean, is_active?: boolean }) => request<void>(`/investments/signals/${id}`, { method: "PATCH", body: JSON.stringify(body) }),
    removeSignal: (id: number) => request<void>(`/investments/signals/${id}`, { method: "DELETE" }),
    getBondSignalFeed: (id: number) => request<{ feed: BondSignalAlert[], signal: { bond: { shortname: string }, [key: string]: unknown } }>(`/investments/signals/${id}/feed`),
